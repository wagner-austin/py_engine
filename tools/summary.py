...
\033[1;33mOptions:\033[0m
  ...
  \033[1;32m-t, --tags\033[0m
    Filter results by these tag(s). Multiple tags => any match triggers inclusion.

  \033[1;32m-s, --search\033[0m
    Case-insensitive search in OS filename, # FileName, version, summary, or tags.
  ...
  \033[1;32m-sj, --save-json\033[0m  [optional filename or no argument]
    Save results as JSON to a file. No argument => auto-naming.

  \033[1;32m-sc, --save-code & --savecode\033[0m  [optional filename or no argument]
    Save full code of matched files to a .txt file, with a blank line separating each file.
...
def main():
    ...
    parser.add_argument("files_pos", nargs="*", help=argparse.SUPPRESS)
    ...
    for summ in all_summaries:
        ...
        print(
            f"\033[92mFile:\033[0m {summ['filename']}  \033[94m(Path:\033[0m {summ['fullpath']}"
            f"\033[94m)\033[0m"
        )
        print(
            f"\033[92mFileName header:\033[0m {summ['header_filename'] or '[None found]'}"
        )
    ...
    # 7) Save results to JSON if requested
    json_file_path = None
    ...
    # 8) Save full code to a .txt file if requested
    if args.save_code is not None:
        ...
        # Get the absolute path of this script to skip saving its own code
        current_file = os.path.abspath(__file__)

        try:
            with open(outfile_code_path, "w", encoding="utf-8") as out_file:
                for summ in all_summaries:
                    # Skip saving this script's own code
                    if os.path.abspath(summ["fullpath"]) == current_file:
                        continue
